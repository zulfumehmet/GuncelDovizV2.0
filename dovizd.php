...
?>' },
  ...
  xAxis: {
    categories: [<?php
						$query = $db->query("SELECT * FROM doviz WHERE kategori = $gelen_deger AND id %3 = 1 ORDER BY `doviz`.`tarih` DESC LIMIT 20", PDO::FETCH_ASSOC);
if ( $query->rowCount() ){
...
?>',
    data: [<?php $query = $db->query("SELECT * FROM doviz WHERE kategori = $gelen_deger AND id %3 = 1 ORDER BY `doviz`.`tarih` DESC LIMIT 20", PDO::FETCH_ASSOC);
if ( $query->rowCount() ){
     foreach( $query as $row ){
